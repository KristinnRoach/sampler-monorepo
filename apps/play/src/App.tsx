...
const App: Component = () => {
  const [controlsRoot, setControlsRoot] = createSignal<HTMLDivElement | null>(null);
  const [layout, setLayout] = createSignal<LayoutType>("desktop");
  const [envHeight, setEnvHeight] = createSignal<number>(225);

  const [currentAudioBuffer, setCurrentAudioBuffer] = createSignal<AudioBuffer | null>(null);
  ...
  const { handleSampleSelect } = useSampleSelection(getSamplePlayer, controlsRoot, setSidebarOpen);
  ...
  onMount(() => {
    ...
    void (async () => {
      try {
        ...
        if (!sample.byteLength) console.warn("Failed to fetch app default sample");
    ...
    enableSamplePlayerMidi({
      ...
      if (success) {
        showNotification("MIDI enabled - Press Cmd+Shift+M to access MIDI Learn");
      } else {
  ...
  return (
    <>
      <div class="content-wrapper">
            ...
            sections={[
              {
                id: "samples",
                title: "",
                content: <SampleListSection onSampleSelect={handleSampleSelect} />,
              },
            ]}
        ...
        <div ref={setControlsRoot} class={`control-grid layout-${layout()}`} id="sampler-container">
