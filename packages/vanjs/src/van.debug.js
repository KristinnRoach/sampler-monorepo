  ...
  get: (vanTags, name) => {
    ...
    return (...args) => {
      const [props, ...children] =
        protoOf(args[0] ?? 0) === Object.prototype ? args : [{}, ...args];
      const debugProps = {};
      for (const [k, v] of Object.entries(props)) {
        ...
        if (isState(v))
          ...
          typeof v === 'function' &&
          (!k.startsWith('on') || v._isBindingFunc)
        )
          debugProps[k] = van.derive(() => validatePropValue(v()));
        else debugProps[k] = validatePropValue(v);
      }
...
const tagsNS = (ns) => {
  expect(
    typeof ns === 'string',
    'Must provide a string for parameter `ns` in `van.tags`'
  );
  return _tagsNS(ns);
};
