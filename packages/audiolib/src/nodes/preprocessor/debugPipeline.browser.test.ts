...
describe('Debug >1.0 peak issue', () => {
  ...
  it('should track where peaks exceed 1.0', async () => {
    ...
    const { normalizeAudioBuffer } = await import('@/utils/audiodata/process/normalizeAudioBuffer');
    const { compressAudioBuffer } = await import('@/utils/audiodata/process/compressAudioBuffer');
    const { detectThresholdCrossing } = await import('@/utils/audiodata/process/detectSilence');
    ...
    expect(peakAfterNorm).toBeLessThanOrEqual(0.98);
    ...
    console.log(`4. Silence detection: start=${start}, end=${end}, buffer.length=${compressed.length}`);
