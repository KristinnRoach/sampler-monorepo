class NoInLoopNode extends AudioWorkletNode {
  constructor(context, options = {}) {
    ...
    const updateThreshold = options.updateThreshold || 0.001;
  ...
  // Connect to an AudioBufferSourceNode to control its loop properties
  ...
  handleProcessorMessage(event) {
    if (event.data && event.data.type === 'update' && this.sourceNode) {
