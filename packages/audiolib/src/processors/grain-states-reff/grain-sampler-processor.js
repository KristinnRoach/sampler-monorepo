class GrainSamplerProcessor extends AudioWorkletProcessor {
  ...
  calculateGainForSampleIndex(grainSampleIndex, parameters, sampleIndex) {
    const attackTime = this.getParameterValue(
      parameters,
      'attack',
      sampleIndex
    );
    const holdTime = this.getParameterValue(parameters, 'hold', sampleIndex);
    const releaseTime = this.getParameterValue(
      parameters,
      'release',
      sampleIndex
    );
  ...
  playGrain(grain, channelIndex, outputChannel, sampleIndex, parameters) {
    ...
    const sampleValue =
      this.sampleBuffers[channelIndex][
        sampleBufferIndex % this.sampleBufferLength
      ];

    // Calculate the gain
    const gain = this.calculateGainForSampleIndex(
      grainChannel.position,
      parameters,
      sampleIndex
    );
    ...
    if (
      grainChannel.position >= grainChannel.length ||
      grainChannel.position === 0
    ) {
      ...
      const smearValue =
        Math.random() *
        this.getParameterValue(parameters, 'spread', sampleIndex);

      // Reset both channels
      const pos = this.getParameterValue(
        parameters,
        'playbackPosition',
        sampleIndex
      );
  ...
  process(inputs, outputs, parameters) {
    ...
    for (
      let channelIndex = 0;
      channelIndex < numOutputChannels;
      ++channelIndex
    ) {
      ...
      for (let sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex) {
        ...
        const density = this.getParameterValue(
          parameters,
          'density',
          sampleIndex
        );
...
registerProcessor('grain-sampler-processor', GrainSamplerProcessor);
