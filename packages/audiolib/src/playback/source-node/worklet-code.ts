export const sourceProcessorCode = `class SourceProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    this.isReleased = false; // Track if node has been released
    ...
    this.port.onmessage = (event) => {
      if (event.data.type === 'setBuffer') {
        ...
        if (this.buffer) {
          const positionInSeconds = Math.max(0, Math.min(event.data.position, this.currentEnd));
          this.playbackPosition = Math.floor(positionInSeconds * this.sampleRate);
        }
      } else if (event.data.type === 'release') {
        // Mark for cleanup and reuse
        this.isReleased = true;
        this.playing = false;
        this.buffer = null;
        
        // Notify main thread this node is ready for reuse
      ...
      for (let i = 0; i < outputChannel.length; i++) {
        ...
        if (this.playbackPosition >= endSample) {
          if (
            ...
            // Notify main thread that playback has ended
            this.port.postMessage({
              type: 'ended',
              reason: 'completed'
            });
