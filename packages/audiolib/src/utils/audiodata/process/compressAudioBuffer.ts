...
export function compressAudioBuffer(
  ctx: AudioContext,
  buffer: AudioBuffer,
  threshold = 0.3,  // Level above which compression starts
  ratio = 4,        // Compression ratio (4:1 means 4dB input = 1dB output above threshold)
  targetMakeupGain = 1.5  // Desired makeup gain (will be reduced if input is already loud)
): AudioBuffer {
  ...
  if (inputPeak > 0) {
    ...
    const compressedPeak = inputPeak <= threshold ? inputPeak : threshold + (inputPeak - threshold) / ratio;
  ...
  for (let ch = 0; ch < numChannels; ch++) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (targetGainReduction < currentGainReduction) {
        ...
        currentGainReduction = Math.min(
          1,
          currentGainReduction + releaseRate
        );
      }
