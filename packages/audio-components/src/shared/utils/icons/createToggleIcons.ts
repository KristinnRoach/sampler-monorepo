...
export const createToggleIcons = (options: { width?: string; height?: string } = {}) => {
  ...
  return {
    ...
    loopOn: createSvgIcon(loopOnSvgRaw, { width, height, color: "white" }) ?? "",
    loopOff: createSvgIcon(loopOffSvgRaw, { width, height, color: "white" }) ?? "",
    record:
    ...
    armed: createSvgIcon(listeningSvgRaw, { width, height, color: "white" }) ?? "",
    stop: createSvgIcon(stopSvgRaw, { width, height, color: "currentColor" }) ?? "",
  };
};
