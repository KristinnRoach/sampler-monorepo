...
import { KnobElement, type KnobChangeEventDetail } from "../primitives/KnobElement";
...
export const SampleControls = (
  ...
  const { container: loopSliderContainer, sliderElement: loopSliderEl } = createSliderGSAP(
    "Trim",
    loopStartSeconds,
    loopEndSliderThumb,
    {
  ...
  const loopEndOffsetKnob = document.createElement("knob-element") as HTMLElement;
  ...
  (loopEndOffsetKnob as HTMLElement).addEventListener("knob-change", (e: CustomEvent) => {
  ...
  const controls = div(
    ...
    div(
      {
        style: "display: flex; place-items: center; column-gap: 0.5rem; margin-top: 10px;",
      },
      loopSliderContainer,
      loopEndOffsetKnob,
    ),
