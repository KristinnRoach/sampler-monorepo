...
import { createCustomEnvelope, type CustomEnvelope } from '@repo/audiolib';
...
const EnvelopeElement = (attributes: ElementProps) => {
  const envelope: CustomEnvelope = createCustomEnvelope();
  ...
  const element = div(
    ...
    (() => {
      ...
      van.derive(() => {
        updateTrigger.val; // Dependency on envelope changes
