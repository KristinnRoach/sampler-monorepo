...
test.describe("Sidebar Accordion", () => {
  ...
  test("can switch between Menu and Saved Samples sections", async ({ page }) => {
    // Open sidebar with Menu section
    await page.getByTitle("Toggle sidebar menu").click();
    await expect(page.getByRole("heading", { name: /sidebar menu/i })).toBeVisible();
    // Menu section should be visible
    await expect(page.locator(".accordion-header", { hasText: "Menu" })).toHaveClass(/open/);
    await expect(page.getByText("Home")).toBeVisible();

    // Switch to Saved Samples section
    await page.getByRole("button", { name: /saved samples/i }).click();
    await expect(page.getByRole("button", { name: /saved samples/i })).toHaveClass(/open/);
    await expect(
      page.getByText("No saved samples").or(page.getByText(/\d{1,2}\/\d{1,2}\/\d{4}/)),
    ).toBeVisible();
    ...
    console.log("Menu header aria-expanded:", ariaExpanded, "class:", menuClass);
    ...
    await page.locator(".accordion-header", { hasText: "Saved Samples" }).click();
    await expect(page.locator(".accordion-header", { hasText: "Saved Samples" })).toHaveClass(
      /open/,
    );
  });
});
